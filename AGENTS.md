...
- Every clickable button must include a descriptive `title` attribute for native tooltips (and matching `aria-label` when icon-only). The HTML `disabled` attribute suppresses native tooltips; use `aria-disabled` with a guarded handler when the control is inert but still needs a tooltip. Decorative icons inside the button need `pointer-events-none` so hover stays on the button.
