...
const knownAliases = CHORD_PRESETS.flatMap((preset) =>
...
export function getPresetIdForSelection(qualityId: string, extensionIds: string[]) {
  return CHORD_PRESETS.find(
    (preset) =>
      preset.qualityId === qualityId &&
      JSON.stringify(preset.extensionIds) === JSON.stringify(extensionIds),
  )?.id
}

export function getChordQueryForSelection(
  root: NoteName,
  qualityId: string,
  extensionIds: string[],
) {
...
function parsePresetAlias(normalized: string, fallback: ChordSelection): ChordSelection | null {
  ...
  const suffix =
    root === fallback.root && !normalized.startsWith(root.toLowerCase())
      ? normalized
      : normalized.slice(consumedRootLength)

  const preset = CHORD_PRESETS.find(
    (item) =>
      item.aliases.some((alias) => alias.toLowerCase() === suffix) ||
      item.label.toLowerCase() === suffix,
  )
